...
Promise.all([
...
]).then(([user, sessionsResult, account]) => {
  ...
  console.log("📋 Accounts table accessible:", !!account);
}).catch(err => {
...
export const authOptions = {
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        try {
          ...
          console.log("👤 User found:", user ? { id: user.id, email: user.email, emailVerified: user.emailVerified } : null);
  ...
  events: {
    async signIn({ user, account, profile, isNewUser }: { user: User; account: Account; profile: Profile; isNewUser: boolean }) {
      try {
    ...
    async signOut({ session, token }: { session: Session; token: any }) {
      try {
        console.log("❌ SignOut event - session:", session?.user?.id);
      } catch (error) {
        console.error("❌ SignOut event error:", error);
      }
    },
    async createSession({ session, token, user }: { session: Session; token: any; user: User }) {
      try {
    ...
    async updateSession({ session, token, user }: { session: Session; token: any; user: User }) {
      try {
    ...
    async deleteSession({ session, token }: { session: Session; token: any }) {
      try {
